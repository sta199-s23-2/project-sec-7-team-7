...
#cholesterol * age
fbs_int_age_reg <- logistic_reg() |>
  set_engine("glm") |>
  fit(num ~ chol * age, data = heart_disease, family = "binomial")

glance(fbs_int_age_reg)$AIC
...
#chol + sex
fbs_add_chol_sex <- logistic_reg() |>
  set_engine("glm") |>
  fit(num ~ chol + sex, data = heart_disease, family = "binomial")

```

The best predictor of heart disease rates is an interactive model of blood pressure and sex because it has the highest adjusted r-squared. The second best predictor is sex itself. The worst predictor of heart disease rates is cholesterol because it has the lowest adjusted r-squared.
